...
export default function DaysUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...daysRecord };
    setProperties(
  ...
  const [daysRecord, setDaysRecord] = React.useState(days);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Days, id) : days;
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Days.copyOf(daysRecord, (updated) => {
