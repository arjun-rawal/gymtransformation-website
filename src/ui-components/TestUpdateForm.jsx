...
export default function TestUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...testRecord };
    setIndex(cleanValues.index);
    setProperties(cleanValues.properties);
    setErrors({});
  };
  const [testRecord, setTestRecord] = React.useState(test);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Test, id) : test;
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Test.copyOf(testRecord, (updated) => {
